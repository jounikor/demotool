/*
 * v0.2 (c) 2024 Jouni 'Mr.Spiv' Korhonen
 * 
 * =======================================================================
 *
 ...
 *
 * Sprites are used to display the 32 color palette. Each sprite is
 * 14 pixels wide and 55 pixels high. We use sprite colors 1 and 2,
 * and copper to change sprite colors every 11 lines. This provides
 * us with 35 individually colored "sprite squares". If there is enough
 * overscan or screen is positioned far left we obviously start losing
 * sprites and the palette goes pooof. So be careful with the horizontal
 * overscan. Experimenting a bit a horizontal start position 56 within
 * "Edit Graphics Size" is the lowest value you get all 8 sprites and
 * full visible palette.
 */
...
struct UCopList* setup_copper(struct Screen* p_scr, cop_cfg_t* p_cfg)
{
    ...
    for (y = 0; y < 5; y++) {
        ...
        CMove(p_ucl,(void*)0xdff1a2,*p_pal++); CBump(p_ucl);   // spr1 col2
        CMove(p_ucl,(void*)0xdff1ac,*p_pal++); CBump(p_ucl);   // spr2 col1
        CMove(p_ucl,(void*)0xdff1aa,*p_pal++); CBump(p_ucl);   // spr3 col2
        CMove(p_ucl,(void*)0xdff1b4,*p_pal++); CBump(p_ucl);   // spr4 col1
        CMove(p_ucl,(void*)0xdff1b2,*p_pal++); CBump(p_ucl);   // spr5 col2
        CMove(p_ucl,(void*)0xdff1bc,*p_pal++); CBump(p_ucl);   // spr6 col1
        CMove(p_ucl,(void*)0xdff1ba,*p_pal++); CBump(p_ucl);   // spr7 col2
    } // 40
    ...
    // Another attemps
    CMove(p_ucl,(void*)0xdff08e,0x1a71); CBump(p_ucl);
